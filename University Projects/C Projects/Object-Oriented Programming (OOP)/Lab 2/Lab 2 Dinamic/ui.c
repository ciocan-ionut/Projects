...
void MainMenu()
{
	...
	printf("5. Afiseaza stocul ordonat\n");
	printf("6. Iesire\n");
...
void showStocMinim(Service* service)
{
	...
	int length;
	MateriePrima* filtru = getMateriiFromServiceByCantitate(service, &length, cantitate);
	if (filtru)
		for (int i = 0; i < length; i++)
		{
...
{
	printf("1. Afiseaza stocul ordonat dupa nume\n");
	printf("2. Afiseaza stocul ordonat dupa cantitate\n");
	printf("3. Inapoi\n");
	printf("Alegeti optiunea: ");
}

void showAscendingMenu()
{
	printf("Cum doriti sa fie sortat?\n");
	printf("1. Crescator\n");
	printf("2. Descrescator\n");
	printf("Alegeti optiunea: ");
}

int chooseAscendingMenu(int* optiune, bool* ascending)
{
	showAscendingMenu();
	if(!intValid(optiune))
	{
...
void showMateriiOrdered(Service* service)
{
	...
	bool ascending;
	intValid(&option);
	MateriePrima* ordonat = NULL;
	switch (option)
	{
	case 1:
		if (chooseAscendingMenu(&optiune, &ascending))
			return;
		ordonat = getMateriiFromServiceOrderedByNume(service, *compareNume, ascending);
		break;
	case 2:
		if (chooseAscendingMenu(&optiune, &ascending))
			return;
		ordonat = getMateriiFromServiceOrderedByCantitate(service, *compareCantitate, ascending);
		break;
	case 3:
		return;
	default:
